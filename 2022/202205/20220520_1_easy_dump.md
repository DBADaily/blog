...
![easydump](images/easydump_v1.png)

While you are upgrading from PostgreSQL earlier releases like PostgreSQL 9.1 to later releases like PostgreSQL 11, `pg_dump` and `pg_upgrade` provided by PostgreSQL is the most reliable and most powerful tool.
...
Apart from the dump functionality, Easy Dump also sends successful notifications and checks errors when dump or restore failed and sends failure notifications with details in case invalid password is given or no proper ACL is given, so that you are informed timely with helpful information.

# Easy Dump introduction

## Easy Dump Overview

Easy Dump is written with library [Easy Bash](https://github.com/dbadaily/easybash) and provides 5 dump modes to supoort 16 most useful cases.
...
Using Easy Bash built-in options `-G` and `-w` to generate a config file and write static varibles into the config file, so that only a few options need to be provided by command line.
...
After confile generated,  command
...
2. You need to minize the dump time to reduce the affect on the application
